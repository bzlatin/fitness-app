...
export default function SquadInviteClient({ raw, code, debug }: SquadInviteClientProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${apiHost}/api/social/squad-invite/${code}`, { signal: controller.signal });
        if (!res.ok) return;
        const data = (await res.json()) as { squadName?: string; memberCount?: number; maxMembers?: number };
        const name = data.squadName?.trim();
        if (name) setSquadName(name);
        if (typeof data.memberCount === 'number') setMemberCount(data.memberCount);
        if (typeof data.maxMembers === 'number') setMaxMembers(data.maxMembers);
      } catch {
        // ignore
      }
    };
    ...
    return (
      <main className="min-h-screen bg-background flex flex-col items-center justify-center px-6 py-12">
        <div className="w-full max-w-md bg-surface border border-border rounded-2xl p-6 text-center">
          ...
          <p className="text-text-secondary mb-6">This squad invite link doesn&apos;t look right.</p>
          {debug ? (
    ...
    <main className="min-h-screen bg-background flex flex-col">
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-text-primary mb-2 tracking-tight text-center">
          Opening your squad invite in Push<span className="text-primary">/</span>Pull
        </h1>

        <p className="text-text-secondary text-center text-base mb-8 max-w-md">
          {squadName ? (
            <>
              <span className="text-text-primary font-semibold">{squadName}</span>
