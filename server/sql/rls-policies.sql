...
CREATE POLICY workout_template_exercises_select_parent_owner
  ...
  USING (
    EXISTS (
      ...
      WHERE wt.id = template_id AND wt.user_id = (select auth.uid()::text)
...
CREATE POLICY follows_select_related
  ...
  FOR SELECT
  USING (
    (select auth.uid()::text) IN (user_id, target_user_id) OR (select auth.role()) = 'service_role'
  );
...
CREATE POLICY workout_sets_select_owner
  ...
  USING (
    EXISTS (
      ...
      WHERE ws.id = session_id AND ws.user_id = (select auth.uid()::text)
    ) OR (select auth.role()) = 'service_role'
  );
...
CREATE POLICY squads_select_accessible
  ...
  USING (
    is_public
    OR EXISTS (
      SELECT 1 FROM squad_members sm WHERE sm.squad_id = id AND sm.user_id = (select auth.uid()::text)
    )
    OR (select auth.role()) = 'service_role'
  );
...
CREATE POLICY squads_insert_owner
  ON squads
  FOR INSERT
  WITH CHECK (created_by = (select auth.uid()::text) OR (select auth.role()) = 'service_role');
...
CREATE POLICY squads_update_owner_or_admin
  ...
  USING (
    ...
    OR EXISTS (SELECT 1 FROM admin_users au WHERE au.user_id = (select auth.uid()::text))
    OR (select auth.role()) = 'service_role'
  )
  WITH CHECK (
    created_by = (select auth.uid()::text)
...
CREATE POLICY squad_members_select_on_squad
  ...
  USING (
    ...
    OR EXISTS (SELECT 1 FROM squads s WHERE s.id = squad_id AND s.created_by = (select auth.uid()::text))
    OR (select auth.role()) = 'service_role'
  );
...
CREATE POLICY squad_invite_links_select_owner
  ...
  FOR SELECT
  USING (
    EXISTS (SELECT 1 FROM squads s WHERE s.id = squad_id AND s.created_by = (select auth.uid()::text))
    OR (select auth.role()) = 'service_role'
  );
  ...
  USING (
    ...
    OR (
      target_type = 'share' AND EXISTS (
        SELECT 1 FROM workout_shares ws WHERE ws.id = target_id AND ws.user_id = (select auth.uid()::text)
      )
    )
    OR (
      target_type = 'status' AND EXISTS (
        SELECT 1 FROM active_workout_statuses aws WHERE aws.session_id = target_id AND aws.user_id = (select auth.uid()::text)
      )
    )
    OR (select auth.role()) = 'service_role'
  );
...
CREATE POLICY user_blocks_select_owner
  ON user_blocks
  FOR SELECT
  USING (blocker_id = (select auth.uid()::text) OR (select auth.role()) = 'service_role');

DROP POLICY IF EXISTS user_blocks_insert_owner ON user_blocks;
CREATE POLICY user_blocks_insert_owner
  ON user_blocks
  FOR INSERT
  WITH CHECK (blocker_id = (select auth.uid()::text) OR (select auth.role()) = 'service_role');
...
CREATE POLICY feedback_items_delete_admin
  ...
  FOR DELETE
  USING (
    EXISTS (SELECT 1 FROM admin_users au WHERE au.user_id = (select auth.uid()::text))
    OR (select auth.role()) = 'service_role'
  );
...
CREATE POLICY feedback_reports_select_self_or_admin
  ...
  FOR SELECT
  USING (
    reported_by = (select auth.uid()::text)
    OR EXISTS (SELECT 1 FROM admin_users au WHERE au.user_id = (select auth.uid()::text))
    OR (select auth.role()) = 'service_role'
  );
...
CREATE POLICY feedback_reports_insert_self
  ON feedback_reports
  FOR INSERT
  WITH CHECK (reported_by = (select auth.uid()::text) OR (select auth.role()) = 'service_role');
