...
const resolveEnvironment = () => APP_STORE_ENV.toLowerCase() === "production" ? app_store_server_library_1.Environment.PRODUCTION : app_store_server_library_1.Environment.SANDBOX;
const getClient = () => {
...
const toStatus = (transaction, renewalInfo) => {
    ...
    const inGrace = renewalInfo?.gracePeriodExpiresDate && renewalInfo.gracePeriodExpiresDate > now && isExpired;
    let status = "active";
    if (inGrace) {
...
const decodeNotification = (signedPayload) => {
    ...
    const transaction = signedTransactionInfo ? decodeSignedPayload(signedTransactionInfo) : null;
    const renewalInfo = signedRenewalInfo ? decodeSignedPayload(signedRenewalInfo) : null;
    return {
        payload,
        transaction,
        renewalInfo,
