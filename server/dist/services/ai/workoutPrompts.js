...
exports.buildSubstitutionPrompt = exports.buildWorkoutGenerationPrompt = exports.formatFatigueScores = exports.formatRecentWorkouts = exports.determineNextInCycle = void 0;
/**
 * Determine the next workout in a split cycle based on recent history
 */
const determineNextInCycle = (recentWorkouts, preferredSplit) => {
...
exports.determineNextInCycle = determineNextInCycle;
/**
 * Format recent workouts for the AI prompt
 */
const formatRecentWorkouts = (workouts) => {
    ...
    return workouts
        ...
        const exerciseList = workout.exercises
            .map((ex) => `  - ${ex.exerciseName}: ${ex.sets} sets × ${ex.avgReps} reps${ex.avgWeight ? ` @ ${ex.avgWeight}lb` : ""}`)
            .join("\n");
        return `${idx + 1}. ${workout.templateName} (${workout.splitType || "custom"}) - ${workout.completedAt}\n${exerciseList}`;
    })
        .join("\n\n");
};
exports.formatRecentWorkouts = formatRecentWorkouts;
/**
 * Format muscle fatigue scores for the AI prompt
 */
const formatFatigueScores = (fatigue) => {
    if (!fatigue) {
    ...
    const scores = Object.entries(fatigue)
        ...
        let status = "";
        if (score < 70)
            status = "✅ Under-trained (prioritize)";
        else if (score < 110)
            status = "🟢 Optimal";
        else if (score < 130)
            status = "🟡 Moderate fatigue";
        else
            status = "🔴 High fatigue (avoid heavy work)";
        return `  - ${muscle}: ${score} (${status})`;
    })
        .join("\n");
    return scores || "No fatigue data available.";
};
exports.formatFatigueScores = formatFatigueScores;
const formatFatigueTargets = (targets) => {
    if (!targets)
    ...
    return `Prioritize: ${prioritize.length > 0 ? prioritize.join(", ") : "auto based on split"}${avoid.length > 0 ? ` | Avoid/limit: ${avoid.join(", ")}` : ""}`;
};
/**
 * Generate the system prompt for workout generation
 */
const buildWorkoutGenerationPrompt = (params, availableExercises) => {
    ...
    const formatEquipmentLabel = (value) => value
        .replace(/_/g, " ")
    ...
    const cardioSummary = cardioPreferences?.enabled
        ...
        : "None";
    const cardioEquipment = profile.availableEquipment?.filter((item) => cardioEquipmentIds.has(item));
    const cardioEquipmentSummary = cardioPreferences?.enabled
        ? cardioEquipment && cardioEquipment.length > 0
            ...
            : "None"
        : "None";
    const requestedSplit = params.requestedSplit ||
    ...
    if (params.specificRequest &&
        ...
        exercisesToUse = availableExercises.filter((ex) => {
            return muscleGroups.some((requestedMuscle) => {
                // Direct match
                if (ex.primaryMuscleGroup.includes(requestedMuscle) ||
                    requestedMuscle.includes(ex.primaryMuscleGroup)) {
                    return true;
                }
                // Check variations
                const variations = muscleGroupVariations[requestedMuscle] || [];
                return variations.some((variation) => ex.primaryMuscleGroup.includes(variation) ||
...
${(0, exports.formatRecentWorkouts)(params.recentWorkouts || [])}

# MUSCLE GROUP FATIGUE ANALYSIS
${(0, exports.formatFatigueScores)(params.muscleFatigue)}
...
1. **Exercise Selection**:
   - Choose 4-8 exercises appropriate for ${params.specificRequest
...
7. **Naming & Labeling**:
   ...
   - If a specific muscle focus is requested (${focusLabel || "none"}), name it after those muscles (e.g., "Arms", "Glutes & Hamstrings")
...
exports.buildWorkoutGenerationPrompt = buildWorkoutGenerationPrompt;
/**
 * Generate prompt for exercise substitution
 */
const buildSubstitutionPrompt = (exerciseName, exerciseMuscle, reason, availableEquipment, availableExercises) => {
...
exports.buildSubstitutionPrompt = buildSubstitutionPrompt;
