...
exports.validateImageBuffer = exports.extractPublicId = exports.deleteImage = exports.uploadImage = void 0;
...
const isModerationSubscriptionError = (error) => {
...
const ensureConfigured = () => {
    ...
    if (!CLOUDINARY_CLOUD_NAME || !CLOUDINARY_API_KEY || !CLOUDINARY_API_SECRET) {
        console.warn("[Cloudinary] Missing credentials. Image uploads will be disabled. Set CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, and CLOUDINARY_API_SECRET in .env");
        return false;
    }
 ...
 */
const uploadImage = (buffer, folder = "custom-exercises", publicId) => {
    if (!ensureConfigured()) {
        throw new Error("Cloudinary is not configured. Image uploads are disabled.");
    }
    const doUpload = (options) => new Promise((resolve, reject) => {
    ...
    return doUpload({ moderation }).catch((error) => {
        if (moderation && isModerationSubscriptionError(error)) {
            console.warn("[Cloudinary] Moderation upload failed (subscription missing). Retrying without moderation.");
 ...
 */
const validateImageBuffer = (buffer, maxSizeBytes = 5 * 1024 * 1024) => {
    // Check file size
    ...
    const isValidFormat = validFormats.some((format) => magicBytes.startsWith(format.signature));
    if (!isValidFormat) {
        return {
            valid: false,
