 ...
 */
const categorizeExercise = (exerciseId: string): ExerciseCategory => {
...
const normalizeDifficultyScore = (value: unknown) => {
  const n = typeof value === "string" ? Number(value) : typeof value === "number" ? value : NaN;
  return Number.isFinite(n) ? n : null;
};

const nudgeIncrement = (baseIncrement: number) => {
 ...
 */
const analyzeExerciseProgression = async (
  userId: string,
  exerciseId: string,
  templateId?: string
  ...
  const sessionData = sessions.map((s) => ({
    ...
    hitTarget:
      parseInt(s.hit_target_count) >= parseInt(s.sets) * 0.75, // 75% of sets hit target
  ...
  const hasDifficultySignal =
    typeof latestDifficultyScore === "number" &&
  ...
  const allExceededReps = last2Sessions.every(
    (s) => s.avgReps > s.targetReps + 1
  );
...
export const getProgressionSuggestions = async (
  ...
  for (const exerciseId of exerciseIds) {
    const suggestion = await analyzeExerciseProgression(
      userId,
      exerciseId,
      templateId
    );
  ...
  const sessionCountResult = await query<{ session_count: string }>(
    `
  ...
  const sessionCount = parseInt(
    sessionCountResult.rows[0]?.session_count || "0"
  );
...
export const applyProgressionSuggestions = async (
  userId: string,
  templateId: string,
  exerciseIds?: string[] // Optional: only apply to specific exercises
): Promise<{ updated: number }> => {
  const progression = await getProgressionSuggestions(userId, templateId);
