...
import { FatigueResult, getFatigueScores } from "./fatigue";
import { getRecentWorkouts } from "./fatigue";
import {
  recommendSmartNextWorkout,
...
const formatSplitLabel = (splitKey?: string, fallback?: string) => {
  if (!splitKey && fallback) return fallback;
 ...
 */
const fetchUserTemplates = async (userId: string): Promise<TemplateRow[]> => {
 ...
 */
const getDaysSinceLastSplit = async (
  userId: string,
  splitKey: string
): Promise<number | null> => {
...
const scoreTemplateMatch = (
  ...
  const nameContainsSplit = splitKeywords.some((keyword) =>
    templateNameLower.includes(keyword)
  );
 ...
 */
const getSplitKeywords = (splitKey: string): string[] => {
  switch (splitKey) {
    case "push":
      return ["push"];
    case "pull":
 ...
 */
const getSplitMuscles = (splitKey: string): string[] => {
  switch (splitKey) {
    case "push":
...
const generateReasoning = (
  ...
  matchedTemplate: UpNextTemplate | null,
  daysSinceLastSplit: number | null,
  fatigueStatus: UpNextRecommendation["fatigueStatus"],
  hasProAccess: boolean
): string => {
  const parts: string[] = [];
  const splitLabel = recommendation.selected.label;
  const preferredSplitLabel = formatSplitLabel(
    recommendation.preferredSplit,
    splitLabel
  );
  ...
  if (daysSinceLastSplit !== null && daysSinceLastSplit >= 3) {
    parts.push(
      `${daysSinceLastSplit} days since last ${splitLabel.toLowerCase()}`
    );
  }
 ...
 */
export const getUpNextRecommendation = async (
  userId: string,
  hasProAccess: boolean
): Promise<UpNextRecommendation> => {
  try {
    ...
    const matchedTemplate =
      scoredTemplates.find((t) => t.matchScore >= 85) ?? null;
    ...
    const daysSinceLastSplit = await getDaysSinceLastSplit(
      userId,
      recommendedSplitKey
    );
