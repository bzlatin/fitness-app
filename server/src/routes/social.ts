...
router.put("/squads/:squadId", async (req, res) => {
  ...
  try {
    ...
    await query(
      `UPDATE squads SET ${updates.join(", ")} WHERE id = $1`,
      [squadId, ...values]
    );
...
router.put("/squads/:squadId/members/:memberId/role", async (req, res) => {
  ...
  try {
    ...
    const ownerResult = await query<{ role: string }>(
      `SELECT role FROM squad_members WHERE squad_id = $1 AND user_id = $2 LIMIT 1`,
      [squadId, userId]
    );
...
router.post("/squads/:squadId/transfer", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      await query(
        `UPDATE squad_members SET role = 'admin' WHERE squad_id = $1 AND user_id = $2`,
        [squadId, userId]
      );
      // Promote new owner
      await query(
        `UPDATE squad_members SET role = 'owner' WHERE squad_id = $1 AND user_id = $2`,
        [squadId, newOwnerId]
      );
      // Update squads.created_by
      await query(
        `UPDATE squads SET created_by = $2, updated_at = NOW() WHERE id = $1`,
        [squadId, newOwnerId]
      );
      await query(`COMMIT`);
    } catch (err) {
...
router.post("/block", async (req, res) => {
  ...
  try {
    ...
    await query(
      `DELETE FROM follows WHERE (user_id = $1 AND target_user_id = $2) OR (user_id = $2 AND target_user_id = $1)`,
      [userId, blockedUserId]
    );
    return res.status(204).send();
  } catch (err) {
...
router.get("/reactions/:targetType/:targetId", async (req, res) => {
  ...
  try {
    // Get emoji counts
    const emojiCounts = await query<{ emoji: string; count: string }>(
      `
    ...
    const userReactions = await query<{ emoji: string }>(
      `
        SELECT emoji
        FROM workout_reactions
